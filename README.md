...
```bash
brew tap haribote1110/tap
brew install --cask stormforge          # Electron版
brew install --cask stormforge-native   # Rustネイティブ版 (軽量・高速, Apple Silicon)
```
...
> **macOSでの初回起動について**: StormForgeは署名なしアプリのため、初回起動時にGatekeeperにブロックされる場合があります。その場合は **システム設定 > プライバシーとセキュリティ** を開き、「このまま開く」をクリックしてください。
